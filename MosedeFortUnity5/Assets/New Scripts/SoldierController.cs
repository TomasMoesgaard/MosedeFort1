...
public class SoldierController : MonoBehaviour {
    ...
    IEnumerator GoToWaypoint()
    {
        ...
        if (path[currentWaypoint].GetComponent<MotionBehavior>().WaitForEventEnd)
        {
                ...
                if(path[currentWaypoint].GetComponent<MotionBehavior>().LookTarget != null)
                {
                    transform.LookAt(path[currentWaypoint].GetComponent<MotionBehavior>().LookTarget.transform);
                }
